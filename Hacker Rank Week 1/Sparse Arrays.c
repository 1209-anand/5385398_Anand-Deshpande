...
int* matchingStrings(int strings_count, char** strings, int queries_count, char** queries, int* result_count) {
    ...
    int *matches = malloc(queries_count * sizeof(int));
    if (!matches) exit(EXIT_FAILURE);

    // For each query, count matches in strings
    for (int q_index = 0; q_index < queries_count; q_index++) {
        int counter = 0;
        for (int s_index = 0; s_index < strings_count; s_index++) {
            if (strcmp(strings[s_index], queries[q_index]) == 0) {
                counter++;
            }
        }
        matches[q_index] = counter;
    }
